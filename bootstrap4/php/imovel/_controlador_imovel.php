...
function add_entidade($conn, $post, $user) {
  ...
  $selection_query = "SELECT * FROM entidades WHERE id = " . $id;
	$update_query = 'UPDATE entidades SET
					...
					estado=' . $state .
			' WHERE id=' . $id;
	$insert_query = 'INSERT INTO entidades(nome,telemovel,pertence_a,morada,email,nascimento,local,zip1,zip2,nif,estado) VALUES(
					"' . $name . '",
          "' . $phnr . '",
          '  . $user . ',
					"' . $addr . '",
...
function adicionar_relacao($conn, $post) {
  ...
  if($exists_same>0) {
    $conn->query("UPDATE entidades_assoc SET estado=1 WHERE entidadeA = " . $contactB . " AND entidadeB = " . $contactA . " AND relacao = " . $relation)->num_rows;
  }
...
function add_imovel_entidades_relation($conn,$imovel,$entidade) {
  ...
  if($conn->query($test_query)->num_rows>0) {
    $query = 'UPDATE imoveis_entidades SET deleted=null WHERE imovel=' . $imovel . ' AND entidade = ' . $entidade;
  }
  else {
...
function delete_imovel_entidades_relation($conn,$imovel,$entidade) {
  $test_query = 'SELECT * FROM  imoveis_entidades WHERE imovel=' . $imovel . ' AND entidade = ' . $entidade;
  if($conn->query($test_query)->num_rows>0) {
    $query = 'UPDATE imoveis_entidades SET deleted="' . date("Y-m-d\TH:i:s") . '" WHERE imovel=' . $imovel . ' AND entidade = ' . $entidade;
  }
...
if(isset($_POST['action_type'])) {
	...
	switch($type){
    case 1: add_imovel_entidades_relation($conn,$_POST['imovel'],$_POST['entidade']); break;
    case 2: delete_imovel_entidades_relation($conn,$_POST['imovel'],$_POST['entidade']); break;

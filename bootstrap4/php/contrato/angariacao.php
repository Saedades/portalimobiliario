...
if(isset($_GET['id'])) {
  ...
  if($aangariacao = $conn->query("SELECT * FROM angariacoes WHERE id =" . $_GET['id'] . " AND iduser=" . $_SESSION['login'])->fetch_assoc()) {
...
//--------------------------------------------------------------  SELECCAO DOS IMOVEIS RELACIONADOS ---------------------------------------------------//
    ...
    //ESTA VERSÃO SELECIONA OS IMOVEIS RELACIONADOS COM ESTA ANGARIAÇÃO DIRECTAMENTE DA TABELA angariacoes_imoveis, MAS TAMBÉM DOS IMOVEIS PERTENCENTES ÀS ENTIDADES ASSOCIADAS
    //$imoveis = $conn->query("SELECT * FROM imoveis WHERE iduser=" . $_SESSION['login'] . " AND id in (SELECT imovel FROM contatos_imoveis WHERE contato IN " . $subquery . " AND deleted_at IS NULL) OR id IN (SELECT idimo FROM angariacoes_imoveis WHERE idang = " . $id . ")")->fetch_all(MYSQLI_ASSOC);
  ...
  //ESTA VERSÃO SELECIONA OS IMOVEIS RELACIONADOS COM ESTA ANGARIAÇÃO DIRECTAMENTE DA TABELA angariacoes_imoveis

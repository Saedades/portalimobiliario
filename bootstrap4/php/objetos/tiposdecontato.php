<?php
	...
	$lista_tipos_contatos = ($result = $conn->query('SELECT * FROM tipo_contato ORDER BY id ASC')) ? $result->fetch_all(MYSQLI_ASSOC) : array();
?>
						...
						if (e.keyCode == 13) {
							...
							$.ajax({
								...
								success: function(result){
									Swal.fire(
									  	'Sucesso!',
									  	'O tipo de contato foi modificado.',
									  	'success'
									);
									$('#'+idcla).text(texto);
									$('#'+idcla).show();
            ...
            <div id="content-wrapper" class="d-flex flex-column">
                <div id="content">
                    ...
                    <div class="container-fluid" style="max-width:600px;margin:unset;">
						...
						<h1 class="h3 mb-4 text-gray-800">Tipos de Contato</h1>
						...
						<div class="form-group" id="classificador_div">
							...
							<ul class="list-group list-group-flush">
								<?php
								foreach($lista_tipos_contatos as $tipo) {
									echo '<li class="list-group-item" style="border:1px solid #caccd9; border-radius:8px">';
									echo '<div class="row">';
									echo '<div class="col-10"><span class="span_classificador" id="'.$tipo['id'].'">'.$tipo['nome'].'<span></div>';
									echo '<div class="col-0">';
									if($tipo['id']!=1)
										echo '<i data-idcla="'.$tipo['id']. '" class="fas fa-trash" style="position: relative;top: -14px;"></i>';
									echo '</div>';
									echo '</div>';
									echo '</li>';
								}
